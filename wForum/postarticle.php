...
function preprocess(){
	...
	if (isset($_GET["reID"])) {
	    ...
	    //if ($boardName == '2014Graduate' && !($currentuser["userlevel"]&BBS_PERM_SYSOP ) )
		  //  foundErr("本版只可回复文章，不可发表文章!");
		$reID = 0;
	}
...
<script type="text/javascript">
		var geetest_passed = false; /* 定义一个全局的变量保存验证结果 */
		function gt_custom_ajax(result) {
			...
			if(result) {
				//alert("yes");
				document.getElementById("oSubmit").disabled = false;
				document.getElementById("oSubmit").value = '发 表';
			} else {
				// alert("");
				document.getElementById("oSubmit").disabled = true;
			}
		}
	</script>
...
<?php 
	for ($i=1; $i<=69; $i++) {
		...
		echo "<img src=\"emot/em".$ii.".gif\" width=30px height=30px border=0 onclick=\"insertsmilie('[em".$ii."]')\" style=\"CURSOR: hand\">&nbsp;";
	} 
?>
            </td>
        </tr>
...
<?php
		...
		echo "<input checked=true type=\"checkbox\" name=\"anonymous\" value=\"1\" />匿名<br />";
	}
	if ($reID == 0) {
?>
...
<BR><BR></td>
	...
	<?php
		//var_dump(isset($_SESSION['firstpostarticle']));
		//var_dump($boardName=="WHUCelebration");
		//var_dump(!($currentuser["userlevel"]&BBS_PERM_SYSOP ));
		if(!isset($_SESSION['firstpostarticle']) && $boardName=="WHUCelebration") {
			echo "<TD align=left class=TableBody1><b>*安全验证</b></TD><TD align=left class=TableBody1><script type=\"text/javascript\" src=\"http://api.geetest.com/get.php?gt=c54da03586a0ed48a03d61778d711f8a\"></script></TD></TR>";
		}
	?>
	
	<td valign=middle colspan=2 align=center class=TableBody2>
	<input type=Submit value='请填写验证码后发表' name=Submit id="oSubmit" disabled = true> &nbsp;&nbsp;&nbsp; <input type=button value='预 览' name=preview onclick=gopreview()>
                </td></form></tr>
      </table>
</form>
...
if(isset($_SESSION['firstpostarticle']) || $boardName != "WHUCelebration") {
	echo "<script type=\"text/javascript\">document.getElementById(\"oSubmit\").disabled = false;document.getElementById(\"oSubmit\").value = '发 表'</script>";
}
?>
