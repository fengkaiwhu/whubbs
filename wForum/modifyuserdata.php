...
<?php 
	$groupinf = getgroupinfo($currentuser['userid']);
	for($i=0;$i<count($groups);$i++) {
		echo "<option value=\"".$groups[$i]."\"";
		if ('无门无派'==$groups[$i]&&$groupinf[1]==''){
			echo " selected ";
		} else if($groupinf[1]==$groups[$i]){
			echo " selected ";
		} 
...
<td width="60%" class="TableBody1"> 
<INPUT maxLength=5 size=44 name=creategroup value="<?php echo $groups[$currentuser['userid']]; ?>">
</td>
</tr><tr> 
...
<td width="60%" class="TableBody1"> 
<INPUT maxLength=5 size=44 name=grouptitle value="">&nbsp;&nbsp;弟子的id：<INPUT maxLength=20 size=10 name=groupmember value="">
</td>
</tr>
<TR> 
...
<?php
	for ($i=1;$i<count($character);$i++) {
		echo "<input type=\"radio\" name=\"character\" value=\"".$i."\" ";
