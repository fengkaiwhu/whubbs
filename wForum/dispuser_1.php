...
<?php $arr = getgroupinfo($user['userid']);if($arr[1]!='') echo $arr[1].' '.$arr[2]; else echo '无门无派'?>
 </b></td>
  ...
  <tr> 
    ...
    <td  width=35%  class=TableBody1><b>
<?php echo bbs_getuserexp($user['userid']); ?>
 </b></td>
    <td class=TableBody1 width=15% align=right>经验等级：</td>
    <td width=35%  class=TableBody1><b><?php echo bbs_getexplevel($user['userid']); ?></b> 
    </td>
  </tr>
  <tr> 
  ...
  <tr> 
    <td class=TableBody1 width=15% align=right>同门：</td>
    <td  width=35%  class=TableBody1 colspan="3"><b>
    <?php $arr = getgroupinfo($user['userid']);if($arr[1]!='') $arr = getgroupmember($arr[1]);for($i=0;$i<count($arr);$i++) {echo "<a href='dispuser.php?id=".$arr[$i]."'>".$arr[$i]."</a>&nbsp;&nbsp;&nbsp;";if($i!=0 && $i%8==0) echo "<br>";}?>
 </b></td>
  </tr>
  <tr> 
