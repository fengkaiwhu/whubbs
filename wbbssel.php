...
if ($keyword) {
	if (bbs_searchboard(trim($keyword),$exact,$boards)) {
		if (sizeof($boards)==1) {
			...
			header("Location: wForum/board.php?name=" . urlencode($boards[0]['NAME']));
			exit;
...
<?php
	foreach ($boards as $board) {
		...
		$col1 = '<a href="wForum/board.php?name='.urlencode($board['NAME']).'">'.htmlspecialchars($board['NAME'],ENT_QUOTES).'</a>';
