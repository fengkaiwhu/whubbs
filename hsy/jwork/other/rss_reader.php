	...
	function end_element($parser, $name){
		...
		switch ($name){
			...
			case "ITEM":
				...
				$this->rss_items[$this->current_channel][$this->current_item]['unix_date']    = (isset($this->tmp_items['date']) && $this->tmp_items['date']) ? @strtotime($this->tmp_items['date']) : time();
				//Check date time
				...
				if ( ($this->rss_items[$this->current_channel][$this->current_item]['unix_date'] <= $tmpdate) && isset($this->tmp_items['date']) ){
					...
					if ( !$this->rss_items[$this->current_channel][$this->current_item]['unix_date'] ){
						$this->rss_items[$this->current_channel][$this->current_item]['unix_date']	= time();
					}
				}
			...
			case "CHANNEL":
				...
				$this->rss_channels[$this->current_channel]['unix_date']	= (isset($this->tmp_channels['date']) && $this->tmp_channels['date']) ? @strtotime($this->tmp_channels['date']) : time();
				//Check date time
				$tmpdate	= 31554000; //1971
				if ( ($this->rss_channels[$this->current_channel]['unix_date'] <= $tmpdate) && isset($this->tmp_channels['date']) ){
					...
					if ( !$this->rss_channels[$this->current_channel]['unix_date'] ){
						$this->rss_channels[$this->current_channel]['unix_date']	= time();
					}
				}
