...
function checkAll_zhaiwx(){
	len_zhaiwx = document.getElementsByTagName('input').length;
	o_zhaiwx = document.getElementsByTagName('input');
	for(var i=0;i<len_zhaiwx;i++){
		if(o_zhaiwx.item(i).type='checkbox')
			o_zhaiwx.item(i).checked = true;
	}
}
function checkother_zhaiwx(){
	len_zhaiwx = document.getElementsByTagName('input').length;
	o_zhaiwx = document.getElementsByTagName('input');
	for(var i=0;i<len_zhaiwx;i++){
		if(o_zhaiwx.item(i).type='checkbox'){
			if(o_zhaiwx.item(i).checked )
				o_zhaiwx.item(i).checked = false;
			else
				o_zhaiwx.item(i).checked = true;
		}
	}
...
docWriter.prototype.t = function() {
	...
	if (this.man) {
		...
		{
			ret += '<input type="button" value="全选" onclick="checkAll_zhaiwx();"/><input type="button" value="反选" onclick="checkother_zhaiwx();"/><input type="button" value="恢复到版面" onclick="mansubmit(' + bbsman_modes['UNDEL'] + ');"/>';
		}
		ret += '<br></form>';
	}
